#> Efeitos de Biomas
execute if score #Game_Phase phase matches 1.. as @a[gamemode=survival] at @s run function trenches:game/general/check_biome

#> Efeitos de Lugares
# A Corrupção (Poison)
execute positioned 232 -5 91 if block ~ ~ ~ minecraft:crying_obsidian as @a[distance=..7,gamemode=!spectator] run effect give @s poison 5 0

# O Cemitério (Cegueira)
execute positioned 169 -2 43 if block ~ ~ ~ minecraft:crying_obsidian as @a[distance=..10,gamemode=!spectator] run effect give @s blindness 2 0
...
#Templo Tropical
execute positioned 332 -6 92 if block ~ ~ ~ minecraft:crying_obsidian as @a[distance=..9,gamemode=!spectator] run function trenches:game/general/temple_effects
...
#> Feedback das Bases
execute unless score #blu_Base value matches 1.. if block 459 3 66 red_wool run function trenches:game/bases/domination {"target":"blu","who_dom":"red"}
execute unless score #red_Base value matches 1.. if block -58 3 66 blue_wool run function trenches:game/bases/domination {"target":"red","who_dom":"blu"}
...
#> First Blood
execute as @a[tag=fb,scores={kp=1..},team=red] run function trenches:game/general/first_blood {"plr_team":"red"}
execute as @a[tag=fb,scores={kp=1..},team=blu] run function trenches:game/general/first_blood {"plr_team":"blu"}
...
#> Player no Spawn
execute as @a[tag=selecting_class] unless data entity @s Inventory[{components:{"minecraft:custom_data":{ui:1b}}}] run give @s compass[custom_name='{"bold":true,"color":"aqua","italic":false,"translate":"trenches.class.selector"}',custom_data={ui:1b},enchantment_glint_override=true,max_stack_size=1]
...
#> Anti-Cheat
execute unless score #Match_End value matches 1.. as @a[team=!spec,gamemode=spectator] at @s if entity @s[y=-1.3,dy=-10] run tp ~ ~5 ~
...
execute as @a[tag=selecting_class] run function trenches:ui/player/tick
